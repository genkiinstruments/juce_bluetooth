...
struct ValueTreeListener : juce::ValueTree::Listener
{
    explicit ValueTreeListener(juce::ValueTree vt)
            : valueTrees({std::move(vt)}) { for (auto& v: valueTrees) v.addListener(this); }

    explicit ValueTreeListener(std::initializer_list<juce::ValueTree> vts)
            : valueTrees(vts) { for (auto& vt: valueTrees) vt.addListener(this); }
    ...
    void valueTreePropertyChanged(juce::ValueTree& vt, const juce::Identifier& id) override { if (property_changed) property_changed(vt, id); }
    void valueTreeChildAdded(juce::ValueTree& parent, juce::ValueTree& child) override { if (child_added) child_added(parent, child); }
    void valueTreeChildRemoved(juce::ValueTree& parent, juce::ValueTree& child, int idx) override { if (child_removed) child_removed(parent, child, idx); }
